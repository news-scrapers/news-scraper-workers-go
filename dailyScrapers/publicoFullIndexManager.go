...
type ElPublicoFullIndexManager struct {
	Config models.ScrapingConfig
	Index  models.ScrapingIndex
}

func (scraper ElPublicoFullIndexManager) ScrapNewsInItems(baseUrl string, scrapingIndex *models.ScrapingIndex) {
	//results := []models.ReviewScraped{}
	urlsPending := []models.UrlNew{}
	newsScraper := PublicoNewsScraper{scraper.Config}
	...
	c.OnHTML("a[class]", func(e *colly.HTMLElement) {
		if e.Attr("class") == "page-link" && strings.Contains(e.Attr("href"), ".html") {
			url := e.Attr("href")
			date := time.Now()
			urlScrap := models.UrlNew{"https://www.publico.es" + url,date}
			fmt.Println(url)
...
func (scraper ElPublicoFullIndexManager) scrapAllReviewsInUrl(urlbase models.UrlNew, reviewsScraper *PublicoNewsScraper, out chan models.NewScraped) models.NewScraped {
